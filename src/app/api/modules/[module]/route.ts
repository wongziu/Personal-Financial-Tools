...
import { insertModuleRecord, updateModuleRecord } from "@/lib/modules";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ module: string }> }) {
  try {
    const { module } = await params;
    const body = (await request.json()) as { rowId?: unknown; values?: Record<string, unknown> };
    const database = getSeededDatabase();
    const row = updateModuleRecord(database, module, Number(body.rowId), body.values ?? {});
    revalidatePath(`/${module}`);
    revalidatePath("/");
    return NextResponse.json({ row });
  } catch (error) {
